...
const AcordionProject = ({ project }: AcordionProps) => {
  ...
  const filteredClients = useMemo(() => {
    if (filter) {
      return project.clients.filter((client: Client) =>
  ...
  return (
    <AccordionRoot variant="subtle" collapsible defaultValue={["b"]}>
      <AccordionItem key={project.id} value={project.id}>
        <Box position="relative">
          <AccordionItemTrigger>
            <BiSolidFolderOpen size={"2em"} color="gray" />
            {project.name}
          </AccordionItemTrigger>
        </Box>
        <AccordionItemContent>
            ...
            <StyledBox spaceY={5}>
              ...
              >
                <HStack gap="5">
                  ...
                  <ProgressBar rounded={"full"} flex="1" />
            ...
            <StyledBox>
              ...
              {noClients ? (
                <Center color={"gray.500"}>
                  There are no associated customers
                </Center>
              ) : noData ? (
                <Center color={"gray.500"}>
                ...
                <Flex w={"full"} flexWrap={"wrap"} gap={4}>
                  {filteredClients?.map((client) => (
                    <ClientProjectInfo key={client.id} data={client} />
                  ))}
                </Flex>
              )}
            </StyledBox>
