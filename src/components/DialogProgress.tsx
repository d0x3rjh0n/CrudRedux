...
import { setProgress } from "@/slices/projectsSlice";
...
const schema = yup.object().shape({
  progress: yup
    .number()
    .positive("The progress must be positive")
    .required("The progress must be required")
    .max(100, "The progres must be low than 100%"),
});
const DialogProgress = ({ id }: Props) => {
  ...
  const onSubmit = handleSubmit(async (data) => {
    dispatch(setProgress([id, data.progress]));
    setOpen(false);
    toaster.create({
      type: "success",
      title: "Success",
      description: "The new progress has been stablished",
    });
    {
      /**
      try {
        ...
        <DialogBody>
          <form onSubmit={onSubmit}>
            ...
            <Flex justifyContent={"space-between"} mt={5}>
              ...
              <Button type="submit" bg="blue.700">
                Update
              </Button>
            </Flex>
          </form>
